...
data World =
  -- | 'World' is constructed with two 'Paddle's, two 'Ball's, a 'ScoreKeeper'
  -- and a 'RunningP' to signify whether it is running or not.
...
-- Each player has its own 'Ball', so a World has two Balls.
--
-- A World has a 'Board', which is a set of 'Brick's. In the future it
-- should be possible to select between different Boards.
--
-- A World has a 'ScoreKeeper' that tracks and displays player scores.
--
...
-- Ball's colour. In the future, Ball ownership should be introduced.
--
-- Brick ownership is also presently signified by checking if the Brick's
-- colour is the same as the ball's colour. This should be changed in the
-- future.
--
...
crunch :: World
-- | 'crunch' destroys a 'World'. Presently it makes a new one as well. In the
-- future, it should only destroy a World.
crunch = bang
...
-- | 'clampTangibles' makes sure the tangibles of a 'World' do not move
-- off-screen.
...
-- | 'clampPaddles' makes sure two 'Paddle's do not move off-screen.
clampPaddles (p1, p2) = (clampPaddle p1, clampPaddle p2)

clampPaddle ::  Paddle -> Paddle
-- | 'clampPaddle' makes sure a 'Paddle' does not move off-screen.
...
-- | 'clampBalls' makes sure two 'Ball's do not move off-screen.
clampBalls (b1, b2) = (clampBall b1, clampBall b2)

clampBall ::  Ball -> Ball
-- | 'clampBall' makes sure a 'Ball' does not move off-screen.
...
-- | 'updateTangibles' moves a world's tangibles one step forward. It reflects
-- and moves all movables and updates the score.
...
-- updates a World's scores and resets the 'Ball's if necessary.
...
-- | 'updateScores' updates the 'ScoreKeeper's. It checks if either 'Ball' has
-- hit the x-edge of the 'World' and if so awards points and resets the ball.
...
-- bounds. If the player's Ball is at the opponent's edge of the 'World', the
-- player is awarded 50 points. If the player's Ball is at the player's edge,
-- the player is deducted 30 points.
...
-- is a collision, the Ball is reflected.
...
-- on the 'Board'. If a Ball collides, it is reflected and the Brick it hits
-- has its 'Health' updated.
...
-- the 'Board'. If there is a collision, the Ball is reflected and the Brick's
-- 'Health' and ownership is updated.
...
-- | 'reflectBrick' checks if a 'Ball' has collided with a 'Brick'. If there is
-- a collision, the Ball is reflected and the 'Brick' is updated.
...
-- Brick reaches zero Health, it is removed from the 'Board'. If the Brick is
-- neutral, it becomes owned by the player that hits it. If the Brick is
-- owned by the player that hits it, it loses Health. If it is hit by the
-- player which does not own it, it gains Health up to its possible
-- 'MaxHealth'.
...
-- | 'launchBall' launches a 'Ball' in a direction that depends on the position of
-- its owner.
